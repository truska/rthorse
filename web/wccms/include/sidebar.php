	...
	<div id='sidebar' class='nav-collapse'>
		<ul class='sidebar-menu' id='nav-accordion'>
					...
					if (count($submenu) > 0) {
						...
						echo "<li id=\"menu-{$option['id']}\" class=\"menu-i\">";
							...
							foreach ($submenu as $suboption) {
								...
								if (!empty($suboption["form"])) {
									$url .= "?frm=" . $suboption["form"];
									$url .= $suboption["var1"] ?? '';
								} else {
									$url .= $suboption["var1"] ?? '';
								}
								...
								echo "<li id=\"submenu-{$suboption['id']}\" class=\"submenu-i\">";
									echo "<a href='{$url}' target='{$suboption["target"]}'>";
										echo $suboption["title"];
									echo "</a>";
								echo "</li>";
							}

							// Replace with above - form = NUll do not add to url 
							/*
								foreach ($submenu as $suboption) {
									echo "<li id=\"submenu-{$suboption['id']}\" class=\"submenu-i\">";
										echo "<a href='{$suboption["url"]}?frm={$suboption["form"]}{$suboption["var1"]}' target='{$suboption["target"]}'>";
											echo $suboption["title"];
										echo "</a>";
									echo "</li>";
								}
			...
			if (isset($user["username"]) && substr($user["username"], -11) === "@truska.com") {
				echo "<li class='menu-i'>";
					...
					echo "</a>";
					echo "<ul class='collapse sub-menu' id='truskaAdminMenu'>";
						echo "<li class='submenu-i'><a href='admin-database-fields.php'>Database Admin</a></li>";
						echo "<li class='submenu-i'><a href='recordBulkUpdatev1.php'>Bulk Record Updater</a></li>";
					echo "</ul>";
				echo "</li>";
			}
				echo "<li class='menu-j'>";
					echo "<a href='".$baseURL."/wccms/logout.php'>";
		...
		echo "<div class='sidebarinfo'>";
			...
			if ($prefs['prefCMSManual']) {
				echo "<a href='{$baseURL}/wccms/docs/{$prefs['prefCMSManual']}' target='_blank'><i class='far fa-file-pdf'></i>&nbsp;&nbsp;Manual</a><br>";
			}
